...
export const transformMediaData = (userMediaItem) => {
    ...
    return {
        ...
        releasePeriod: essentialData.releasePeriod, // Alterado para releasePeriod
...
export const normalizeSearchResults = (results, mediaType) => {
    ...
    if (mediaType === 'game') {
        return results.map(game => {
            return {
                id: game.id,
                title: game.name,
                description: game.description, // Já vem da API corrigida
                coverImage: game.coverImage,
                // Garantir que platforms seja um array de strings
        ...
        return results.map(item => {
            ...
            // USAR genres completos do TMDB (já vêm com names)
        ...
        return results.map(item => {
            ...
            const baseItem = {
                ...
                // CORREÇÃO: Já está no formato correto para animes/mangás
    ...
    return results.map(item => ({
        ...
        apiVoteCount: item.scored_by || item.ratingCount || item.apiVoteCount || 0,
        releasePeriod: extractReleasePeriod(item),
        // CORREÇÃO: Garantir que genres seja array de objetos
        genres: Array.isArray(item.genres)
            ? item.genres.map(g => ({
                id: g.id || 0,
                name: typeof g === 'string' ? g : g.name || ''
