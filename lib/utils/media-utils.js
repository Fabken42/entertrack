...
export const normalizeSearchResults = (results, mediaType) => {
    ...
    if (mediaType === 'game') {
        return results.map(game => ({
            ...
            releasePeriod: game.releasePeriod || (game.released ? {
                year: new Date(game.released).getFullYear(),
                month: game.released ? new Date(game.released).getMonth() + 1 : undefined
            } : undefined), // Alterado para releasePeriod
            genres: game.genres || [],
        ...
        return results.map(item => {
            ...
            return {
                ...
                releasePeriod: releasePeriod, // Alterado para releasePeriod
        ...
        return results.map(item => {
            ...
            // Extrair releasePeriod dos dados do Jikan
    ...
    return results.map(item => ({
        ...
        apiVoteCount: item.scored_by || item.ratingCount || item.apiVoteCount || 0,
        releasePeriod: extractReleasePeriod(item) // Usa a nova função
    }));
};